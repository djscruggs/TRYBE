...
import {
  type CheckinReminderMailerProps,
  mailChallengeContent,
  sendCheckinReminder
} from '../utils/mailer'
import {
  generateUrl,
  ...
  await Promise.all(
    posts.map(async (post) => {
      if (post.challenge?.members) {
        await Promise.all(
          post.challenge?.members.map(async (member) => {
            const cohortPath = member.cohortId ? `/${member.cohortId}` : ''
            const postPath = pathToEmailUrl(
              `/challenges/v/${post.challengeId}/chat${cohortPath}#featured-id-${post.id}`
            )
            ...
            const props = {
              ...
              dynamic_template_data: {
                ...
                // body: post.body
                body: textToHtml(
                  convertYouTubeLinksToImages(post.body ?? '', postLink)
                )
              }
            }
            try {
...
export const sendDayNumberPosts = async (): Promise<{
  dayNumberPosts: number
  dayNotifications: number
}> => {
  ...
  if (posts.length === 0) {
    ...
    await Promise.all(
      Object.values(dayNumberHash)
        .flat()
        .map(async (member) => {
          const cohortPath = member.cohortId ? `/${member.cohortId}` : ''
          const checkinPath = pathToEmailUrl(
            `/challenges/v/${member.challenge.id}/checkins${cohortPath}`
          )
        ...
        if (members) {
          await Promise.all(
            members.map(async (member) => {
              ...
              const cohortPath = member.cohortId ? `/${member.cohortId}` : ''
              const postPath = pathToEmailUrl(
                `/challenges/v/${post.challengeId}/chat#featured-id-${post.id}${cohortPath}`
              )
              const postLink = generateUrl(postPath)
              const props = {
                to: member.user.email,
  ...
  return {
    dayNumberPosts: posts.length,
    dayNotifications: nonPostNotifications
  }
}
