...
export const handleFormUpload = async ({
  formData,
  dataObj,
  nameSpace,
  onUpdate
}: FormUploadProps): Promise<DataObj> => {
  ...
  try {
    if (image ?? formData.get('image') === 'delete') {
      ...
      if (image) {
        ...
        const imgMeta = await saveBufferToCloudinary(
          Buffer.from(buffer),
          imgNoExt
        )
  ...
  try {
    if (video ?? formData.get('video') === 'delete') {
      ...
      if (video) {
        ...
        const videoMeta = await saveBufferToCloudinary(
          Buffer.from(buffer),
          vidNoExt
        )
