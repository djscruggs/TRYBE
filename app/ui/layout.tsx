  ...
  return (
    <>
      ...
      <div className="md:hidden px-1  flex flex-col min-h-screen min-w-screen p-0">
        <div
          className={`flex flex-col items-center justify-center  ${isWelcome ? "p-0" : " px-2"}`}
        >
          <Outlet />
          {showSpacer && (
        ...
        {showNav && !isChat && (
          <div
            className={`${wrapperVisible ? "opacity-100" : "opacity-30"}  transition-opacity duration-500 fixed bottom-0 left-0 right-0 max-w-screen flex items-center w-full justify-center m-0 p-0 px-2 py-1 bg-gray-50 border-2 border-slate-200 z-10`}
          >
            <div className="max-w-lg flex justify-between w-full">
              <div
                className={`flex  items-center  ${currentUser ? "w-1/3 justify-between" : "w-1/2 justify-center  "}`}
              >
                {currentUser && (
                  <Link
                    to="/home"
              ...
              <div
                className={`flex justify-center items-center ${currentUser ? "w-1/3" : "hidden"}`}
              >
                {currentUser && (
              ...
              <div
                className={`flex justify-center items-center ${currentUser ? "w-1/3" : "w-1/2"}`}
              >
                {currentUser ? (
                  <Link
                    to="/profile"
                    className="w-8 h-8 mr-10 flex justify-center items-center"
                  >
