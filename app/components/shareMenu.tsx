...
export default function ShareMenu (props: ShareMenuProps): JSX.Element {
  const { copyUrl, itemType, itemId, isPreview, className, noText } = props
  ...
  return (
    <div className={`relative ${className}`}>
      <div className="flex items-center">
        ...
        {!noText && <span className="cursor-pointer text-xs inline" onClick={handleShareMenu}>Share</span>}
      </div>
      {showMenu &&
        <div className='cursor-pointer w-32 absolute left-0 bottom-8 bg-white border border-gray rounded-md flex flex-col text-left' >
          ...
          <p className='text-black text-sm hover:bg-gray-100 p-1' onClick={copyLink}>Copy Link</p>
        </div>
      }
    </div>
  )
