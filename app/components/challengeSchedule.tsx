...
export default function ChallengeSchedule({
  ...
  return (
    <div
      className={`flex-col  border-red items-center w-screen max-w-2xl  ${isSchedule ? 'md:max-w-xl lg:max-w-2xl' : 'md:max-w-md lg:max-w-lg'}`}
    >
...
const DateSchedule = ({
  ...
  const postsByDayNum = posts.reduce<Record<number, Post[]>>((acc, post) => {
    ...
    const startAt = challenge.startAt
      ? new Date(challenge.startAt as unknown as Date)
      : new Date()
    const day = differenceInDays(date, startAt) + 1 // Calculate days since challenge.startAt
    if (day <= 0) {
      unscheduled.push(post)
    } else {
      if (!acc[day]) {
  ...
  return (
    <div
      className={`px-2 w-full max-w-lg  ${isSchedule ? 'md:max-w-xl lg:max-w-2xl' : 'md:max-w-md lg:max-w-lg'}`}
    >
      <div
        className={`${isSchedule ? 'md:grid' : ''}  grid-cols-7 gap-0 w-full mt-4 `}
      >
        {/* only show the days if we're on the schedule page */}
        {weekDays.map((day) => (
          <div
            key={day}
            className={`hidden ${isSchedule ? 'md:block' : ''} text-center font-bol`}
          >
            {day}
          </div>
        ))}
...
const NumberSchedule = ({
  ...
  return (
    <div
      className={`w-full max-w-screen px-4 md:px-0  ${isSchedule ? 'md:max-w-xl lg:max-w-2xl' : 'md:max-w-md lg:max-w-lg'}`}
    >
      <div
        className={`${isSchedule ? 'md:grid' : ''}  grid-cols-7 gap-2 w-full mt-4`}
      >
        {Array.from({ length: numDays }, (_, index) => (
          <div
            key={index}
            className="flex flex-col items-center justify-center p-2 border border-gray-300 text-center  relative  h-24 bg-lightgrey  border-[#CECECE]'"
          >
...
const NewPostLink = ({
  ...
  const newPost = (): void => {
    ...
    const state = {
      title: `Day ${day}`,
      publishAt: startDate
        ? format(addDays(startDate, day - 1), 'yyyy-MM-dd 08:00:00')
        : null,
      dayNumber: day,
      notifyMembers: true
    }
