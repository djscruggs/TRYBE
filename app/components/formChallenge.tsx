...
function FormChallenge({
  ...
  const frequencies: Array<Challenge['frequency']> = [
    'DAILY',
    'WEEKDAYS',
    'WEEKLY'
  ]
  ...
  const defaults = {
    deleteImage: false,
    numDays: 30,
    type: 'SCHEDULED' as ChallengeType,
    frequency: 'DAILY' as Challenge['frequency'],
    categories: [],
  ...
  const localDateFormat =
    currentUser?.locale === 'en-US' ? 'M-dd-YYYY' : 'dd-M-YYYY'
  ...
  function handleSelect(value: string | undefined): void {
    if (
      value &&
      ['DAILY', 'WEEKDAYS', 'ALTERNATING', 'WEEKLY', 'CUSTOM'].includes(value)
    ) {
      setFormData((prevFormData) => ({
        ...prevFormData,
  ...
  function parseErrors(errors: any, path: string = ''): Record<string, string> {
    ...
    for (const key in errors) {
      if (key === '_errors') {
        ...
        const nestedErrors = parseErrors(
          errors[key],
          path ? `${path}.${key}` : key
        )
  ...
  async function handleSubmit(event: React.FormEvent): Promise<void> {
    ...
    if (formData.type === 'SCHEDULED') {
      if (!formData.startAt) {
        validation.startAt = 'Start date is required'
      }
      if (!formData.endAt) {
        validation.endAt = 'End date is required'
      }
    } else {
      if (!formData.numDays) {
        validation.numDays = 'Number of Days is required'
      }
    }
    if (!formData.icon) {
      validation.icon = 'Icon is required'
    }
    if (!formData.categories || formData.categories?.length === 0) {
      validation.categories = 'Category is required'
    }
    ...
    for (const key in formData) {
      ...
      if (key === 'categories') {
        toSubmit.append(
          key,
          JSON.stringify(formData.categories?.map((category) => category.id))
        )
  ...
  function handleCategoryChange(e: ChangeEvent<HTMLInputElement>): void {
    ...
    const currentCategoryIds = formData.categories?.map(
        ...
        >
          <div className="w-full max-w-150 md:max-w-300 px-2 grid grid-cols-1 md:grid-cols-2  ">
            <div className="col-span-2 w-full lg:col-span-1">
              ...
              <div className="max-w-100 relative flex mb-4">
                ...
                />
                <Select value={formData.frequency} onValueChange={handleSelect}>
              ...
              <fieldset className="border-grey border rounded-md p-2 pb-4 mb-4 max-w-100">
                {currentUser?.role === 'ADMIN' && (
                  <>
                    ...
                    <p className="text-xs">
                      Scheduled challenges happen on specific dates.
                      Self-directed challenges can be started at any time.
                    </p>
                    ...
                    <div className="relative flex flex-col mb-2 md:flex-row md:space-x-2">
                      <div className="relative max-w-[160px] mr-2">
                        ...
                        <DatePicker
                          ...
                          selected={
                            formData.startAt ? new Date(formData.startAt) : null
                          }
                          onChange={(date: Date) => {
                            selectDate('startAt', date)
                          }}
                      ...
                      <div className="relative max-w-[160px] mr-2">
                        ...
                        <DatePicker
                          ...
                          minDate={
                            formData.startAt
                              ? addDays(new Date(formData.startAt), 7)
                              : addDays(new Date(), 7)
                          }
                          selected={
                            formData.endAt ? new Date(formData.endAt) : null
                          }
                          onChange={(date: Date) => {
                            selectDate('endAt', date)
                          }}
              ...
              <div className="max-w-100 relative flex flex-wrap">
                ...
                {colorOptions.map((option, index) => (
                  <div
                    ...
                    className={`w-10 h-10 cursor-pointer rounded-full bg-${colorToClassName(option, 'red')} mr-2 mb-2 ${formData.color === option ? 'outline outline-2 outline-offset-2 outline-darkgrey' : ''}`}
                  ></div>
                ))}
              </div>
                    ...
                    {Object.entries(iconFiles).map(([img, width]) => (
                      <DropdownMenuItem key={img}>
                        <img
                          ...
                          onClick={() => {
                            handleIconChange(img)
                          }}
                          className={`cursor-pointer ${formData.icon === img ? ' outline outline-2  outline-darkgrey rounded-md' : ''}`}
                        />
                      </DropdownMenuItem>
                    ))}
                  </DropdownMenuContent>
          ...
          <div className="mt-8 flex justify-left">
            ...
            <button
              ...
              className="underline ml-4 4 hover:text-red"
            >
              cancel
            </button>
          </div>
...
const CoverPhotoHandler = ({
  ...
  formData: Partial<ChallengeInputs>
  setFormData: (formData: Partial<ChallengeInputs>) => void
  image: File | null
  setImage: (image: File | null) => void
}): JSX.Element => {
  const [imageURL, setImageURL] = useState<string | null>(
    formData.coverPhotoMeta?.secure_url
      ? String(formData.coverPhotoMeta.secure_url)
      : null
  )
  ...
  return (
    <>
      <div
        className={`max-w-md mb-2 h-60 rounded-md flex items-center justify-center ${imageURL ? '' : 'bg-blue-gray-50'}`}
      >
        {imageURL && (
          <>
            <img
      ...
      <div className="px-[28%] justify-center items-center">
        {imageURL && (
          <>
            ...
            <div
              onClick={removeImage}
              className="underline ml-[130px] -mt-8 cursor-pointer"
            >
              remove
            </div>
          </>
