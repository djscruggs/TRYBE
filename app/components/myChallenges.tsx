...
export default function MyChallenges(props: MyChallengesProps): JSX.Element {
      ...
      const userChallenges = allChallenges.filter(
        (challenge) =>
          userMemberships.some(
            (membership) => membership.challengeId === challenge.id
  ...
  return (
    <div
      className={`mb-8 w-full ${centered ? "flex-col justify-center items-center" : ""}`}
    >
      <div
        className={`text-lg w-full relative ${centered ? "text-center" : ""}`}
      >
        ...
        {!loading && myChallenges.length === 0 && memberships.length === 0 ? (
          <>
            <p className="text-left text-gray-500">
              It&apos;s A Little Quiet Here... Ready To Spark Some Action?
            </p>
            <div
            ...
            >
              <button
                ...
                onClick={() => {
                  scrollToBrowse
                    ? scrollToBrowse()
                    : gatedNavigate("/challenges", false);
                }}
              >
                BROWSE CHALLENGES
              </button>
