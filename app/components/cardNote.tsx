...
import type { Note, ChallengeSummary } from "~/utils/types";
      ...
      {editing ? (
        <>
          ...
          {note.challenge && !isThread && (
            <div className="mt-2">
              <CardChallenge
                challenge={note.challenge as ChallengeSummary}
                isShare={true}
              />
            </div>
          )}
        ...
        <div className={"mt-2 w-full border-0  drop-shadow-none mr-2"}>
          <div className="drop-shadow-none">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card
                className={
                  "md:col-span-2 p-2 drop-shadow-lg  border border-gray rounded-md"
                }
              >
                <div className="flex items-start">
                  ...
                  <div className="flex flex-col w-full h-full">
                    {noteBody}
                    <div className="mt-4">
                      {note.videoMeta?.secure_url && (
                        <video
                          className="recorded"
                          src={note.videoMeta.secure_url}
                          onClick={(event) => {
                            event?.stopPropagation();
                          }}
                          controls
                      ...
                      {note.imageMeta?.secure_url && (
                        <img
                          src={note.imageMeta.secure_url}
                          alt="note picture"
          ...
          {currentUser && addReply && (
            <div className="mt-2 w-full border-0  drop-shadow-none mr-2">
              <FormNote
                ...
                prompt="Add your response"
              />
            </div>
          )}
        </div>
      )}
      {note.imageMeta?.secure_url && showLightbox && (
        <Lightbox
          medium={note.imageMeta.secure_url}
          large={note.imageMeta.secure_url}
          alt="note photo"
          onClose={() => {
