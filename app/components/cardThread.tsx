...
export default function CardThread(props: CardThreadProps): JSX.Element {
  ...
  const handleDelete = (event: any): void => {
    ...
    event.stopPropagation();
    axios
      .delete(`/api/threads/delete/${thread.id}`)
      .then(() => {
  ...
  return (
    <>
        ...
        <div className={"mt-2 w-full border-0  drop-shadow-none mr-2"}>
          ...
          >
            <div className="w-lg">
              <Card
                className={
                  "md:col-span-2 p-2 border drop-shadow-lg border-gray rounded-md relative"
                }
              >
                <div className="flex items-start">
                  ...
                  <div className="flex flex-col w-full h-full">
                    ...
                    <div className="mt-4">
                      {thread.videoMeta?.secure_url && (
                        <video
                          className="recorded"
                          src={thread.videoMeta.secure_url}
                          onClick={(event) => {
                            event?.stopPropagation();
                          }}
                          controls
                      ...
                      {thread.imageMeta?.secure_url && (
                        <img
                          src={thread.imageMeta.secure_url}
                          alt="thread picture"
          ...
          <hr />
          <div
            className={`grid ${isOwnRoute ? "grid-cols-2" : "grid-cols-3"} text-center py-2 cursor-pointer`}
          >
            {!isOwnRoute && (
      ...
      {thread.imageMeta?.secure_url && showLightbox && (
        <Lightbox
          medium={thread.imageMeta?.secure_url}
          large={thread.imageMeta?.secure_url}
          alt="thread photo"
          onClose={() => {
